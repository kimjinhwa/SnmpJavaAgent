...
/**
 * The {@code TemporaryList} implements a list whose items are automatically removed after a predefined timeout.
...
public class TemporaryList<T> {
    ...
    private final LinkedList<TemporaryListItem<T>> list = new LinkedList<TemporaryListItem<T>>();
    ...
    class TemporaryListItem<T> {
        ...
        private final long timeOfMaturity;

        public TemporaryListItem(T item) {
            this.item = item;
            this.timeOfMaturity = System.nanoTime() + ((long) timeout * SnmpConstants.MILLISECOND_TO_NANOSECOND);
        }
         ...
         *
         * @return {@code true} if the item is subject to removal from list,
    ...
    class TemporaryListIterator<T> implements Iterator<T> {
        ...
        private final Iterator<TemporaryListItem<T>> iterator;
