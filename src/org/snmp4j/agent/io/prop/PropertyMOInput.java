...
/**
 * The <code>PropertyMOInput</code> can be used to load MIB data from a properties file.
 * <p>
 ...
 * length if the # is used in the format string otherwise no length will be included.</li>
...
public class PropertyMOInput implements MOInput {
    ...
    private ContextInfo scanContexts(Map<String, String> properties) {
        String ctx = properties.get(CONFIG_PREFIX + CONTEXTS_ID);
        StringTokenizer st = new StringTokenizer(ctx, ",");
        ArrayList<Context> l = new ArrayList<>(st.countTokens());
    ...
    private OIDInfo scanOIDs(SortedMap<String, String> properties, OctetString ctx) {
        ...
        for (Entry<String, String> stringStringEntry : oids.entrySet()) {
            Entry<String, String> e = stringStringEntry;
            String k = e.getKey();
    ...
    private void scanDataVariable(SortedMap<String, String> properties, OID oid, DataInfo<Variable> dataInfo) {
    ...
    private void scanDataIndexVariables(SortedMap<String, String> properties, OID oid,
                                        DataInfo<IndexedVariables> dataInfo) {
        String prefix = CONFIG_PREFIX;
        String ctxSuffix = "";
        if (dataInfo.context != null) {
            ctxSuffix = CTX_ID + dataInfo.context;
    ...
    /**
     * Returns the update mode, which might be one of the constants defined by {@link ImportMode}. By default, {@link
     * ImportMode#RESTORE_CHANGES} is returned.
     *
     ...
     *
     * @return {@code null} if {@code value} is {@code null} and the {@code Variable} corresponding to {@code value}
     * otherwise.
     */
    ...
    private class ContextInfo implements Iterator<Context> {
        int numContexts;
        Context curContext;
        Iterator<Context> iterator;

        ContextInfo(int numContexts, Iterator<Context> contexts) {
    ...
    private class OIDInfo implements Iterator<MOInfo> {
        int numElements;
        MOInfo curOID;
    ...
    private class DataInfo<T> implements Iterator<T> {
        OctetString context;
