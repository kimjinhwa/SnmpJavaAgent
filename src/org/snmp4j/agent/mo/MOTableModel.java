/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - MOTableModel.java  
  _## 
...
/**
 * The {@code MOTableModel} interface defines the base table
 * model interface needed for {@code MOTable}s. This model can be used
...
public interface MOTableModel<R extends MOTableRow> {
    ...
    /**
     * Returns {@code true} if the table model contains no rows.
     *
     * @return {@code true} if the table model contains no rows.
     * @since 2.2
     */
    boolean isEmpty();
     ...
     *
     * @return {@code true} if this model has a row of with index
     * {@code index} or {@code false} otherwise.
     */
     ...
     *         the row index.
     *
     * @return the {@code MOTableRow} with the specified index and
     ...
     *
     * @return an {@code Iterator} returning {@code MOTableRow} instances.
     */
    Iterator<R> iterator();

    /**
     * Returns an iterator on a view of the rows of this table model
     * whose index values are greater or equal {@code lowerBound}.
     *
     * @param lowerBound
     *         the lower bound index (inclusive). If {@code lowerBound} is
     *         {@code null} the returned iterator is the same as returned by
     *         {@link #iterator()}.
     *
     * @return an {@code Iterator} over the
     */
     ...
     *
     * @return the {@code MOTableRow} with the smallest index or {@code null}
     ...
     *
     * @return the {@code MOTableRow} with the greatest index or {@code null}
     * if the model is empty.
     */
    R lastRow();
}
